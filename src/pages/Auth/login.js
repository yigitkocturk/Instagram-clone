  ...
  return (
    <div className="h-full w-full flex flex-wrap overflow-auto items-center gap-x-8 justify-center">
      ...
      <div className="hidden md:block w-[380px] h-[581px] bg-logo-pattern relative bg-[length:468.32px_634.15px] bg-[top_left_-46px]">
        <div className="w-[250px] h-[538px] absolute top-[27px] right-[15px]" ref={ref}>
          {images.map((image, key) => (
            <img className="w-full h-full absolute top-0 left-0 opacity-0 transition-opacity duration-1000" src={image} alt="" />
          ))}

        </div>
      </div>
      <div className='w-[350px] grid gap-y-3'>
        <div className='w bg-white border p-[40px] pb-6 pt-8'>
          ...
          <Formik
            ...
            {({ isSubmitting, isValid, dirty, values }) => (
              <Form className='grid gap-y-1.5'>
                ...
                <Seperator />
                <a href='/#' className='flex justify-center items-center gap-x-2 mb-2.5 text-sm font-semibold text-facebook'>
                  <AiFillFacebook size={20} />
                  Log in with Facebook
                </a>
                <a href='/#' className='text-xs flex item-center justify-center text-link'>
                  Forgat password?
                </a>
              </Form>
            )}
